...
- A line above a file's first line says who last changed it and how long ago - `Ada Fischer, 3 days
  ago` - and clicking it narrows the graph to that file. One `git log -1` per file, kept until a
  commit or a checkout in that repository could change it, and let go of when the file is closed;
...
- **Rebase the current branch onto … , interactively…**, on a commit's or a branch's menu: the same
  confirmation the plain rebase gives, then the list. VS Code is named as git's editor for that one
  command only, so no setting of yours changes
...
- Folders: refs that share a prefix are gathered under it in **Branches & Tags** and the header's
  branch menu, where they start closed and open while filtering - `feature/` at every

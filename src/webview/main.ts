...
/**
 * The search box and the date range. What they ask for is the bar's; what the answer looks like -
 * the rows, the marking, the selection - is the view's, and comes back through here.
 */
filterBar.connect({
  post: (message) => vscode.postMessage(message),
  remember: () => saveViewState(),
  rehighlight: () => refreshHighlight(),
  jump: (query) => {
    if (!looksLikeCommitId(query)) {
      return false;
    }
...
/** The column widths and the grips. Everything a column move then has to redo is the view's. */
columns.connect({
  remember: () => saveViewState(),
  redraw: () => schedule(),
...
function laneWidth(): number {
  ...
  const dragged = columns.graphRoom();

  if (dragged !== null) {
    return Math.min(dragged, laneNeed);
  }
...
function render(): void {
   ...
   */
  columns.reposition(`${indent}:${frame.width}:${frame.scrollbar}`);
...
function renderRows(indent: number, first: number, last: number): void {
  ...
  for (let i = first; i < last; i++) {
    ...
    date.textContent = columns.showsTime()
...
window.addEventListener('message', (event: MessageEvent<HostMessage>) => {
  ...
  switch (message.type) {
    ...
    case 'showHistory':
      filterBar.history(message.path);
      break;
...
function refreshHighlight(): void {
  const next = marking(filterBar.search());
